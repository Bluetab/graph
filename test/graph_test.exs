defmodule GraphTest do
  ...
  describe "A graph" do
    ...
    test "tree?/1 returns true iff a graph is a tree", %{g: g} do
      assert Graph.tree?(g)
      refute Graph.tree?(Graph.add_edge(g, :spqr, :xyzzy))
    end

    @tag edges: [foo: :bar, foo: :baz, bar: :xyzzy, baz: :spqr]
    test "arborescence?/1 returns true if a graph is an arborescence", %{g: g} do
      assert Graph.arborescence?(g)
    end
